...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		if(od->od_ino !=0)
		{
			...
			for(llen=0;llen<OSPFS_MAXNAMELEN;llen++)
			{
				if(od->od_name[llen]=='\0')
				break;
			}
			if(entry_oi->oi_ftype == OSPFS_FTYPE_REG)
				ok_so_far = filldir(dirent, od->od_name, llen, f_pos, od->od_ino, DT_REG);
			else if(entry_oi->oi_ftype == OSPFS_FTYPE_DIR)
				ok_so_far = filldir(dirent, od->od_name, llen, f_pos, od->od_ino, DT_DIR);
			else if(entry_oi->oi_ftype == OSPFS_FTYPE_SYMLINK)
				ok_so_far = filldir(dirent, od->od_name, llen, f_pos, od->od_ino, DT_LNK);
			else {
				return -EIO; // error
...
{
	/* EXERCISE: Your code here */
	// search free-block bitmap, mark and return block number
	uint32_t blk_no,vec_no=0,bit_no=0;
	// OSPFS_BLKSIZE_BITS=10
	// OSPFS_FREEMAP_BLK=2
		...
		for(vec_no=0; vec_no<OSPFS_BLKBITSIZE/32; vec_no++)
			for(bit_no=0; bit_no < 32; bit_no++)
			{
				if(bitvector_test (add + vec_no, bit_no)){
					bitvector_clear(add + vec_no ,bit_no);
					return (blk_no - OSPFS_FREEMAP_BLK)* OSPFS_BLKBITSIZE + vec_no*32 + bit_no;
				}
			}
	}
...
add_block(ospfs_inode_t *oi)
{
	...
	// need indirect 
		...
		int is_allocated=0;
		if (oi->oi_indirect ==0){
			// allocate a new block for indirect to save entry
	...
	// need indirect2
		...
		int is_allocated_1 = 0, is_allocated_2 = 0;
		uint32_t allocate_t ;
		...
		allocate_t= allocated[0][(n - OSPFS_NDIRECT - OSPFS_NINDIRECT)/OSPFS_NINDIRECT];
		if (allocate_t == 0)
		{
			allocate_t=allocate_block();
			is_allocated_2=1;
			
			// fail-->free previous allocation
			...
			allocated[0][(n - OSPFS_NDIRECT - OSPFS_NINDIRECT)/OSPFS_NINDIRECT]=allocate_t;
			allocated[1]=ospfs_block(allocate_t);
			memset(allocated[1], 0, OSPFS_BLKSIZE);
		}
	// 2nd level pointer
		allocated[1]=ospfs_block(allocate_t);
	    allocate_t =allocated[1][(n - OSPFS_NDIRECT - OSPFS_NINDIRECT) % OSPFS_NINDIRECT];
		if (allocate_t == 0)
		{
			allocate_t=allocate_block();
			allocated[1][(n - OSPFS_NDIRECT - OSPFS_NINDIRECT) % OSPFS_NINDIRECT]=allocate_t;
			// fail
			if (allocate_t == 0)
			{
				if (is_allocated_2)
				{
					free_block(allocated[0][(n - OSPFS_NDIRECT - OSPFS_NINDIRECT) / OSPFS_NINDIRECT]);
					if (is_allocated_1)
						free_block(oi->oi_indirect2);
				}
				return -ENOSPC;
		...
		//successful
		// data pointer
		allocated[0] = ospfs_block(allocate_t);
		memset(allocated[0], 0, OSPFS_BLKSIZE);
...
remove_block(ospfs_inode_t *oi)
{
	...
	// deal with indirect
		...
		if(oi->oi_indirect==0)
			return -EIO;
		allocated[0] =ospfs_block(oi->oi_indirect);
		if(allocated[0][n-OSPFS_NDIRECT-1]==0)
			return -EIO;	
		free_block(allocated[0][n-OSPFS_NDIRECT-1]);
		allocated[0][n-OSPFS_NDIRECT]=0;
		if(allocated[0][0]==0)
		{
			free_block(oi->oi_indirect)	;
			oi->oi_indirect=0;
	...
	// deal with indirect2
		...
		if(oi->oi_indirect2 ==0)
			return -EIO;
		allocated[0] =ospfs_block(oi->oi_indirect2);
		if(allocated[0][(n-OSPFS_NDIRECT-OSPFS_NINDIRECT-1)/OSPFS_NINDIRECT]==0)
			return -EIO;
		allocated[1]=ospfs_block((allocated[0][(n-OSPFS_NDIRECT-OSPFS_NINDIRECT-1)/OSPFS_NINDIRECT]));	
		if(allocated[1][(n-OSPFS_NDIRECT-OSPFS_NINDIRECT-1)%OSPFS_NINDIRECT]==0)
		return -EIO;
		
		free_block(allocated[1][(n-OSPFS_NDIRECT-OSPFS_NINDIRECT-1)%OSPFS_NINDIRECT]);
		allocated[1][(n-OSPFS_NDIRECT-OSPFS_NINDIRECT-1)%OSPFS_NINDIRECT]=0;
		
		if(allocated[1][0]==0){
			free_block(allocated[0][(n-OSPFS_NDIRECT-OSPFS_NINDIRECT-1)/OSPFS_NINDIRECT]);
			allocated[0][(n-OSPFS_NDIRECT-OSPFS_NINDIRECT-1)/OSPFS_NINDIRECT]=0;
		}
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	for (offset = 0 ; offset < dir_oi -> oi_size ; offset+=OSPFS_DIRENTRY_SIZE)
	{
		...
		retval = change_size (dir_oi, new_size); //implement this function 
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	struct ospfs_inode_t * new_inode =NULL;
	//struct ospfs_symlink_inode *new_inode = 0;
	...
	new_entry = create_blank_direntry (dir_oi); //implement function 
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	ospfs_symlink_inode_t *link = NULL;
